function s(name,number:number):string{
    let arr:string[] =['a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v','w','x','y','z'];
    return name=name.toLowerCase().split('').map(function(e){
        if(arr.includes(e)){
            if(arr.indexOf(e)+number>25)  return arr[arr.indexOf(e)+number-26]
            else if(number<0&&arr.indexOf(e)+number<0) return arr[arr.indexOf(e)+number+26]
            else return arr[arr.indexOf(e)+number]        
        }
        else return e;
    }).join('').toUpperCase()
}

let number_input=document.querySelector('#selected_number') as HTMLInputElement,
    sb_mark = document.querySelector('#sb_mark') as HTMLDivElement,
    text_input=document.querySelector('#textarea_input') as HTMLTextAreaElement,
    text_output =document.querySelector('#output') as HTMLDivElement;
